...
export default `
window.ReactNativeWebView.postMessage(JSON.stringify({ type: "epubjs", message: "[EPUBJS]" }));
...
/* harmony import */ var _xmldom_xmldom__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(16);
...
class Queue {
  constructor(context) {
    ...
    this.tick = _core__WEBPACK_IMPORTED_MODULE_1__["requestAnimationFrame"];
    this.running = false;
    this.paused = false;
  }
  /**
  ...
  enqueue() {
    ...
    if (typeof task === "function") {
      deferred = new _core__WEBPACK_IMPORTED_MODULE_1__["defer"]();
  ...
  dequeue() {
    ...
    if (this._q.length && !this.paused) {
      ...
      if (task) {
        // console.log(task);
        // messageToReact(\`[dequeue::queue] task \${JSON.stringify(task)} \${task}\`);
      ...
      inwait = new _core__WEBPACK_IMPORTED_MODULE_1__["defer"]();
  ...
  run() {
    if (!this.running) {
      this.running = true;
      this.defered = new _core__WEBPACK_IMPORTED_MODULE_1__["defer"]();
    }
...
class continuous_ContinuousViewManager extends managers_default["a" /* default */] {
  ...
  } // TODO THIS FUNCTION IS THE ANWSER
...
var rendition = __webpack_require__(17);
...
var localforage = __webpack_require__(24);
...
var isValue = __webpack_require__(19);
